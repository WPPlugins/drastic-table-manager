...
var drasticGrid = new Class ({
    options: {
		key: "", // 2008-08-17 - jgbustos - WP Nonce key
		table_name: "", // 2008-08-16 - jgbustos
	...
	initialize: function(container, options){
		...
		for (var i=0; i < t.columns.length; i++)  {
			...
			// 2008-08-17 - jgbustos - Added explicit width style to make FF3 happy
		...
		var row = new Element('tr').inject(tbody);
		var td = new Element('td', {'colspan': t.columns.length+1, styles:{'text-align': 'left'}}).inject(row);
	...
	getMetaData: function() {
		...
		}).send('tab=' + t.options.table_name + '&key=' + t.options.key + '&op=vm');
	},
	...
	UpdateCell:	function(obj, value) {
		var t = this;
		// 2008-08-16 - jgbustos - Added call to html_entity_decode
		if (value == html_entity_decode(obj.ddoldvalue)) {t.restore_form(obj, null); return}
		var id = obj.ddid;
		var colname = obj.ddcolumn.name;
		// 2008-08-16 - jgbustos - Replace encodeURI with encodeURIComponent to deal with forward slashes
		var url = t.options.path+'?tab=' + t.options.table_name + '&key=' + t.options.key + '&op=u&id='+obj.ddsqlid+'&col='+colname+'&value='+encodeURIComponent(value);
	...
	create_form: function(obj){
		...
		if (obj.ddcolumn.type == DDTYPEENUM) {
			...
			// 2008-08-16 - jgbustos - Added the call to html_entity_decode
			sel = new Element('input', {'type': 'text',	'value': html_entity_decode(obj.ddvalue)}).inject(obj);
			sel.setStyle('width', obj.ddcolumn.width);			
			sel.addEvent('keydown', function(e){
				var me = new Event(e);
				if (me.key == 'enter') t.UpdateCell(me.target.getParent(), me.target.value);
	...
	tr_del: function(obj) {
		...
		if (t.do_delete) {
			var url = t.options.path+'?tab=' + t.options.table_name + '&key=' + t.options.key + '&op=d&id='+sqlid;
			t.showLoading();
			var myajax = new Request({
				url: url,
				method: 'get',
	...
	tr_add: function(obj) {
		...
		var url = t.options.path + '?tab=' + t.options.table_name + '&key=' + t.options.key + '&op=a';
		t.showLoading();
		var myajax = new Request({
			url: url,
			method: 'get',
	...
	refresh: function() {
		...
		var url = t.options.path+'?tab=' + t.options.table_name + '&key=' + t.options.key + '&op=v&start='+start+'&end='+end+'&cols='+t.cols+'&sortcol='+t.options.sortcol+'&sort='+t.options.sort;
		t.showLoading();
		var myajax = new Request({
			url: url,
			method: 'get',
	...
	DefaultOnClick: function(id) {
		...
		}).send('tab=' + t.options.table_name + '&key=' + t.options.key + '&op=vrn&id='+id);		
	},
