...
class drasticSrcMySQL {
	...
	function __construct($server, $user, $pw, $db, $table, $options = null) {
		...
		$idresult = mysql_query("SELECT * FROM $this->table LIMIT 1", $this->conn);
		$primary_found = false;
		...
		// 2008-08-16 - jgbustos - Added the verification using isset() and the ternary operator
		...
		if (isset($_REQUEST["id"])){
			...
			if ($start < $this->num_rows) {		
				...
				for ($i = 0; $i < ($end-$start); $i++) {
					$value = mysql_fetch_array($this->result);
					for ($j = 0; $j < ((isset($cols))?(count($cols)):($this->num_fields)); $j++) {
						//2008-08-16 - jgbustos - added htmlentities
						$row[$j] = htmlentities($value[((isset($cols))?($cols[$j]):($j))]); 
						//echo $row[$j];
					}
					$arr[$i] = $row;
